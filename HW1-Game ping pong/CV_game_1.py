...
def draw_game():
    ...
    for c in range(3):
        img[y1:y2, x1:x2, c] = img[y1:y2, x1:x2, c] * (1 - racket_alpha / 255.0) + racket_rgb[:, :, c] * (racket_alpha / 255.0)
    ...
    ball_alpha = ball_resized[:, :, 3]
    for c in range(3):
        img[y1:y2, x1:x2, c] = img[y1:y2, x1:x2, c] * (1 - ball_alpha / 255.0) + ball_resized[:, :, c] * (ball_alpha / 255.0)
